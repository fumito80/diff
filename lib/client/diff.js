...
function getUnifiedResult(source, headL, headR) {
  ...
  const [head] = recurse(([, parent]) => {
    return !!parent && headL.x >= m - parent.x - 1 && headL.y >= n - parent.y - 1;
  }, ([pathR, parent = {
    parent: undefined
  }]) => [pathR, parent.parent])([headR, headR.parent]);
...
class UnifiedResultL extends UnifiedResult {
  ...
  getUndiff(x, undiffs) {
    if (undiffs > 0) {
      return [this.makeElem(this.a.slice(x - undiffs, x), ElemType.common)];
    }
...
class UnifiedResultR extends UnifiedResult {
  ...
  getUndiff(x, undiffs) {
    if (undiffs > 0) {
      return [this.makeElem(this.a.slice(this.m - x, this.m - x + undiffs), ElemType.common)];
    }
...
function diff(a, b) {
  ...
  const headR = onp(nR + 3, snakeConditionR); // log(headL, headR);
