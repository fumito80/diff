...
function unifiedResultR({
  ...
  function getAcc([undiff, diffOrNull, [diff], acc]) {
  ...
  return result; //   function getAcc([undiff, diffOrNull, [diff], [prev, ...tail], acc]: Ses[][]): Ses[] {
  //     if (prev && ((prev.added && diff.added) || (prev.removed && diff.removed))) {
...
function Onp(source, offset, delta, rangeKN, rangeKM) {
  return (nMax, snakeCondition) => {
    ...
    const paths = [];
    const fp = new Array(m + nMax + 3).fill({
      "fp": -1,
      "k": -1
    });
...
function diff(a, b) {
  ...
  log(headL, headR);
  return [...unifiedResult(source, headL), ...unifiedResultR(source, headR)]; // return unifiedResultR(source, headR);
}
