...
function unifiedResult({
  ...
  return pathList.reduceRight((acc, {
    ...
    const diffY = y - parent.y; // const ses = [...getDiff(diffX - diffY, parent), ...getUndiff(x, Math.min(diffX, diffY))] as [Ses, Ses?];

    const undiff = getUndiff(x, Math.min(diffX, diffY));
    const diff = getDiff(diffX - diffY, parent);
    const [last] = acc.slice(-1);
    const [next] = [...diff, ...undiff];
    ...
    return [...acc, ...diff, ...undiff];
  }, []);
  return pathList.reduce((acc, {
    x,
    y,
    parent = {
      x: 0,
    ...
    const undiff = getUndiff(x, Math.min(diffX, diffY));
    const diff = getDiff(diffX - diffY, parent); // const ses = [...getUndiff(x, Math.min(diffX, diffY)), ...getDiff(diffX - diffY, parent)] as [Ses, Ses];

    const [last] = acc.slice(-1);
    const [next] = [...undiff, ...diff];
