...
exports.default = {
  // constructor() {
  //   // this.append(this.empty())(this.empty());
  // }
  ...
  concat: function (xss) {
    var self = this;
    return self.match(xss, {
      empty: _ => {
        return self.empty();
      },
      cons: (xs, xss) => {
        return self.append(xs)(xss);
      }
    });
  },
  last: function (alist) {
  ...
  drop: function (list) {
    ...
    return n => {
      ...
      // expect(n).to.be.greaterThan(-1);
      if (n === 0) return list;else {
        if (self.isEmpty.bind(self)(list)) return self.empty;else {
          var tail = this.tail;
